...
export class DrawingGateway implements OnGatewayInit, OnGatewayConnection {
  ...
  constructor(
    @InjectRepository(DrawingContentRepository)
    private readonly drawingContentRepo: DrawingContentRepository,
    @InjectRepository(DrawingRepository)
    private readonly drawingRepo: DrawingRepository,
    @InjectRepository(DrawingEditionRepository)
    private readonly drawingEditionRepository: DrawingEditionRepository,
  ...
  notifyAllUsers(drawing: Drawing) {
    ...
    const drawingInformationsString = JSON.stringify(drawingInformations);
  ...
  async diffuseDrawing(@MessageBody() drawing: any) {
    ...
    if (drawingMod.status === DrawingStatus.Done.valueOf()) {
      await this.drawingContentRepo.update(drawingMod.id, {
        content: drawingMod.content,
        toolName: drawingMod.toolName,
      });
    }
    ...
    this.wss
      .to(drawingMod.drawingId.toString())
      .emit('drawingToClient', drawing);
  }
  ...
  async joinDrawing(client: Socket, dto: any) {
    ...
    if (
      passwordMatch ||
      drawing.visibility === visibility.PUBLIC ||
      drawing.visibility === visibility.PRIVATE
    ) {
      const user = await this.userRepo.findOne(dtoMod.userId);
      const newEditionHistory = new DrawingEditionHistory();
      ...
      const drawingRet = this.drawingRepo.findOne({
        where: [{ id: dtoMod.drawingId }],
      ...
      const drawingString = JSON.stringify(drawingRet);
  ...
  async leaveDrawing(client: Socket, dto: any) {
    const dtoMod: LeaveDrawingDto = JSON.parse(dto);
    let user = await this.userRepo.findOne(dtoMod.userId, {
      select: ['totalCollaborationTime'],
      relations: ['drawingEditionHistory'],
    });
    const editionHistoryDate =
      user.drawingEditionHistories[
        user.drawingEditionHistories.length - 1
      ].date.toString();
    const timeEllapsed: number =
      new Date().getTime() - new Date(editionHistoryDate).getTime();
    const timeEllapsedInSeconds = timeEllapsed / 1000;
    await this.userRepo.update(dtoMod.userId, {
      totalCollaborationTime:
        user.totalCollaborationTime + timeEllapsedInSeconds,
      status: Status.ONLINE,
    });
    user = await this.userRepo.findOne(dtoMod.userId, {
