...
import { ColorConvertingService } from 'src/app/services/color-picker/color-converting.service';
...
export class MiniColorPickerComponent implements OnInit {
  ...
  setColor(): void {
    ...
    this.color = this.colorConvert
      .hsvToHex(
        this.hue,
        this.saturation / this.svMaxValue,
        this.value / this.svMaxValue
      )
      .slice(1);
    this.emitColor();
  }
  ...
  hueSelect(event: MouseEvent): void {
    ...
    this.hue = Math.round(
      (event.offsetY - this.offsetY) *
        (this.cData.MAX_HUE_VALUE / this.svMaxValue)
    );
    this.setColor();
  }
  ...
  get hueCursorStyles(): {} {
    return {
      transform: `translateY(${
        ...
        (this.offsetY - this.hueCursorWidth / 2)
      }px)`,
    };
  }
  ...
  get svCursorStyles(): {} {
    return {
      transform: `translate(${this.saturation}px,${
        this.svMaxValue - this.value
      }px)`,
    };
  }
}
