...
export class MouseHandler {
  ...
  callObserverDown(e: MouseEvent, isRightClick: boolean): void {
    // console.log("DOWN");
    this.observers.forEach((element: InputObserver) => {
      if (element.selected) {
        element.down(
  ...
  callObserverOutsideCanvas(e: MouseEvent): void {
    this.observers.forEach((element: InputObserver) => {
      if (element.selected) {
        element.goingOutsideCanvas(e, this.mouseCanvasPosition);
      }
    });
  }

  callObserverInsideCanvas(e: MouseEvent): void {
    // console.log("InsideCanvas");
    this.observers.forEach((element: InputObserver) => {
      if (element.selected) {
        element.goingInsideCanvas(e, this.mouseCanvasPosition);
      }
    });
  }

  callObserverUp(e: MouseEvent): void {
    // console.log("UP");
    this.observers.forEach((element: InputObserver) => {
      if (element.selected) {
        element.up(e, this.mouseCanvasPosition, this.insideWorkspace);
      }
    });
  }
